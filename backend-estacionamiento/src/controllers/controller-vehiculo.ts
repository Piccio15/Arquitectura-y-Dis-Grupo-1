...
export const VehiculoController = {
  ...
  try {
    ...
    console.log("VEHICULOS:", vehiculos); // agregá esto
    res.json(vehiculos);
  ...
  registrar: async (req: Request, res: Response) => {
    ...
    if (!userId) { res.status(401).json({ error: 'Autenticacion requerida' }); return; }
    try {
      const vehiculo = await VehiculoService.registrarVehiculo(userId, String(req.body.patente));
  ...
  if (!userId) { res.status(401).json({ error: 'Autenticacion requerida' }); return; }
  try {
    const patente = String(req.params.patente);  // ← esto lo soluciona
    await VehiculoService.eliminarVehiculo(userId, patente);
